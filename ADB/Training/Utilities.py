...
# MAGIC         count_previous=spark.sql(f"""select operationMetrics.numOutputRows from Table_count where version<(select max(version) from Table_count where lower(trim(operation))=lower('{operation_type}')) order by version desc limit 1""")
